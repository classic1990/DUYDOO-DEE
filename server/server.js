...
try {
  ...
  console.log(
    "⚠️ ไม่พบ Environment Variable, กำลังพยายามใช้ไฟล์ Local JSON...",
  );
  try {
    serviceAccount = require("./classic-e8ab7-firebase-adminsdk-fbsvc-8c07b33104.json");
  } catch (e) {
    console.error(
      "❌ CRITICAL ERROR: ไม่พบไฟล์กุญแจ Firebase และไม่มี Environment Variable",
    );
    serviceAccount = null;
  }
}
...
if (!admin.apps.length) {
  if (serviceAccount) {
    ...
    console.warn(
      "⚠️ Firebase ไม่ได้ถูก Initialize (ระบบฐานข้อมูลจะใช้งานไม่ได้)",
    );
  }
}
// -------------------------------------------------------------------
...
async function generateWithRotation(prompt) {
  if (apiKeys.length === 0)
    throw new Error("ไม่พบ API Key ของ Gemini (กรุณาตั้งค่า GEMINI_API_KEYS)");
  ...
  while (attempts < apiKeys.length) {
    try {
      ...
      try {
        ...
        console.log(
          `🤖 AI Request: Using 'gemini-1.5-flash' (Key Index: ${currentKeyIndex})...`,
        );
      ...
      if (
        ...
        console.warn(
          `⚠️ Key [${currentKeyIndex}] ใช้งานไม่ได้/โควต้าเต็ม! กำลังสลับ Key ถัดไป...`,
        );
...
const authorizeAdmin = (req, res, next) => {
  ...
  if (req.user && req.user.role === "admin") {
    if (req.user.user.toLowerCase() !== allowedAdmin.toLowerCase()) {
      return res.status(403).json({
        success: false,
        message: "Access Denied: คุณไม่ใช่เจ้าของระบบตัวจริง",
      });
    }
    return next();
  }
  return res
    .status(403)
    .json({ success: false, message: "เฉพาะแอดมินเท่านั้น" });
};
...
apiRouter.post("/login", async (req, res) => {
  try {
    ...
    if (!username || !password)
      return res
        .status(400)
        .json({ success: false, message: "กรุณากรอกข้อมูล" });
    ...
    // [BAN CHECK] ตรวจสอบสถานะการแบน
    if (userData.isBanned) {
      return res.status(403).json({
        success: false,
        message: "บัญชีของคุณถูกระงับการใช้งาน (Banned)",
      });
    }
    ...
    if (
      finalRole === "admin" &&
      userData.username.toLowerCase() !== superAdminEmail.toLowerCase()
    ) {
      finalRole = "user";
    }
    ...
    const refreshToken = jwt.sign({ id: userDoc.id }, process.env.JWT_SECRET, {
      expiresIn: "7d",
    });
...
apiRouter.post("/refresh-token", async (req, res) => {
  ...
  try {
    ...
    if (snapshot.empty) {
      ...
      return res.status(403).json({
        success: false,
        message: "Token ถูกใช้งานไปแล้ว (Reuse Detected)",
      });
    }
...
apiRouter.post("/register", async (req, res) => {
  try {
    ...
    if (password.length < 6)
      return res
        .status(400)
        .json({ success: false, message: "รหัสผ่านสั้นเกินไป" });
...
apiRouter.get("/movies", async (req, res) => {
  try {
    const snapshot = await db
      ...
      .get();
    const movies = snapshot.docs.map((doc) => ({ _id: doc.id, ...doc.data() }));
...
apiRouter.put(
  ...
  async (req, res) => {
    try {
      const { role, isBanned, vipExpiresAt } = req.body;
      ...
      if (role && role === "admin") {
        return res.status(400).json({
          success: false,
          message: "ระบบจำกัดให้มี Admin เพียงคนเดียว (เจ้าของ)",
        });
      }
      ...
      // รองรับการอัปเดตวันหมดอายุ VIP
      if (vipExpiresAt) {
        updateData.vipExpiresAt = admin.firestore.Timestamp.fromDate(
  ...
  async (req, res) => {
    try {
      if (req.params.id === req.user.id) {
        return res
          .status(400)
          .json({ success: false, message: "ไม่สามารถลบตัวเองได้" });
      }
...
// [CHECK API KEYS] - ตรวจสอบสถานะ Key ทั้งหมด
apiRouter.get(
  "/admin/check-keys",
  authenticateToken,
  authorizeAdmin,
  async (req, res) => {
    try {
      const results = [];
      // วนลูปตรวจสอบทุก Key ที่มีในระบบ
      for (const [index, key] of apiKeys.entries()) {
        const genAI = new GoogleGenerativeAI(key);
        // ใช้โมเดล flash ในการเทสเพราะเร็วและประหยัด
        const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });
        ...
        try {
          ...
          results.push({
            ...
            status: "active",
            message: "ใช้งานได้ปกติ",
          });
        } catch (error) {
          let status = "error";
          let message = "ใช้งานไม่ได้";
          if (error.message.includes("429")) {
            status = "quota";
            message = "โควต้าเต็ม (429)";
          } else if (
            error.message.includes("403") ||
            error.message.includes("API key not valid")
          ) {
            status = "invalid";
            message = "คีย์ไม่ถูกต้อง/ไม่มีสิทธิ์";
          }
          results.push({ key: key.substring(0, 10) + "...", status, message });
        }
      }
...
apiRouter.get("/announcement", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: doc.exists ? doc.data() : { text: "", isActive: false },
    });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
...
apiRouter.get("/cron/cleanup", async (req, res) => {
  ...
  try {
    ...
    for (const doc of snapshot.docs) {
      ...
      count++;
      if (count >= 400) {
        // Commit ทุกๆ 400 รายการเพื่อความปลอดภัย
        await batch.commit();
        batch = db.batch();
        totalDeleted += count;
        count = 0;
